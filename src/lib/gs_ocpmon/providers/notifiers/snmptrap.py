    ...
    def notify(cls, message, severity, alertgroup='', alerttype='snmp', installer='',
        ...
        if os.path.exists(config_file):
            ...
            try:
                ...
                logger.error(
                    "Traphost not specified in notify.json OR Host-Snmp Configuration File , Agent cannot send Snmptrap Messages")
            try:
        ...
        # TODO: below is for debug purpose. need to revise the host/ip detect function for ESXi
        host = "localhost.AMAX.COM"
        host_ip = "192.168.139.220"
        ...
        alert_message = "Severity:{severity},Alerttype:{alerttype},Host:{host},Platform:{platform},Message:{message} %{vars_str}".format(
            **locals())
        print("alert_message : " + alert_message)
        logger.debug(locals())
        command = """snmptrap -v 1 -c {community} {traphost} {enterprise_oid} {host_ip} {generic_trap} {specific_trap} {uptime} {oid} s '{alert_message}'""".format(
            **locals())
